    ...
    @isset ($subtotalPrice)
        @if (sprintf("%.2f", $subtotalPrice) != 0)
            ...
            @if (session('cart')['active_group_id'] == 0)
                @if (session('cart')['business_verification'] != null || session('id'))
                    <a href="{{ url('checkout?order_hash='.session('cart')['order_hash']) }}" class="btn style3 place-order-btn" style='padding:0px;'>Place Order</a>

                @else
                    <a href="{{ route('verify-bussiness.index') }}" class="btn style3 place-order-btn" style='padding:0px;'>Place Order</a>

                @endif
            @else
                <a href="#cart-popup" data-toggle="modal" class="btn style3 place-order-btn active-order-group" style="color: grey; padding:0px;"> Place Order</a>

            @endif
        @else
            <a href="#cart-popup" data-toggle="modal" class="btn style3 place-order-btn place-order" style="color: grey; padding:0px;">Place Order</a>
        @endif
    @endisset
    <div class="drop-con">

        <button type="button" class="close mobile-cart-close-btn">
            <span aria-hidden="true">×</span>
        </button>
        <strong>Selected Cart Items</strong>
                            ...
                            <table>
                                ...
                                @isset($cart['plan'])
                                    @if ($cart['plan']['amount_onetime'] > 0)
                                        ...
                                        <td>$ {{ $cart['plan']['amount_onetime'] }}</td>
                                    @endif
                                @endisset
                            </table>
                        </div>
        ...
        <div class="total">
            ...
            @if(!session('id') && session('cart') && !session('changePlanStatus') == 'Upgrade' && isset(session('cart')['business_verification']) && session('cart')['business_verification'] != null)
                <a href='{{ route('clear.order') }}' class='btn' style="background: #d9534f;" id='clear-order' onclick="return confirm('Are you sure?')">Cancel Order</a>
            @endif
        </div>
        {{-- @isset ($subtotalPrice)
            @if (sprintf("%.2f", $subtotalPrice) != 0)
...
@push('js')
    <script>
        $('.mobile-cart-close-btn').on('click',function(e){
            $('#cart-drop-mobile').removeClass('show');
